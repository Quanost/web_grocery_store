...
const SelectGroup = ({ variant, handleSelected, disabel }) => {
    ...
    useEffect(() => {
        if (handleSelected)
            handleSelected(selectedVariant)
    }, [selectedVariant])
    return (
        <div>
             <div className={`flex items-center justify-center bg-slate-200 border border-solid rounded-sm ${disabel ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div class="flex flex-wrap gap-3">
                        {variant?.map((variantItem) => (
                            <label class="cursor-pointer">
                                ...
                                <div className={`w-auto max-w-xl rounded-md bg-white p-2 text-gray-600 ring-2 ring-transparent transition-all hover:shadow peer-checked:text-sky-600 peer-checked:ring-blue-400 peer-checked:ring-offset-2 ${disabel ? 'text-sky-600 ring-blue-400': ''}`}>
